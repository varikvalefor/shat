...
\begin{itemize}
	...
	\begin{itemize}
		\item ga jonai ga je la'oi .\B{b}.\ du la'oi .\IC{nothing}.\ gi tu'a da rinka tu'a lo ctaipe be la'oi .\AgdaRecord{Buffer}.\ be'o poi lo mu'oi zoi.\ \AgdaField{Buffer.lerpinste}\ .zoi.\ be ke'a cu konkatena fi lo mu'oi zoi.\ \AgdaField{Buffer.lerpinste}\ .zoi.\ be la'oi .\B{x}.\ fe ko'a goi lo'i ro lerpinsle pe lo datnyvei poi la'oi .\B{a}.\ cmene ke'a ku'o je poi nilzilcmi ko'a fa lo mu'oi zoi.\ \AgdaField{Buffer.cablerpinsle}\ .zoi.\ be ke'a gi
...
module ReadMaybe'Veritas where
  ...
  nada s nj np = begin
    readMaybe' s ≡⟨ refl ⟩
    if (s ≡ᵇ "$") _ (mapₘ just $ readMaybe s) ≡⟨ ifnon nj ⟩
    ...
    pav x nin inn = begin
      ...
      ninal {xs = z ∷ zs} n = ∉⇒≢ n 𝕃.All.∷ ninal (∉⇒∉₋₁ {xs = zs} n)
        ...
        ... | yes d = ≡⇒∈ _ _ xs (sym d) ⇒⇐ ∉⇒¬∈ _ (x ∷ xs) nin
          ...
          ≡⇒∈ e x xs d = begin
            1 ≡⟨ {!!} ⟩
            𝕃.length (𝕃.take 1 $ 𝕃.filter (e ≟_) (𝕍.toList $ 𝕍.fromList $ x ∷ xs)) ∎
            where
            open ≡-Reasoning
          ∉⇒¬∈ : ∀ {a} → {A : Set a}
               → ⦃ _ : Eq A ⦄
...
module ReedVeritas where
  ...
  private
    ...
    k₂ : (x : Buffer) → (_ _ : Buffer.F x) → Char → String
  ...
  ac x a = sym $ begin
    ...
    rms≡[pK>>=fℕ?] = sym $ begin
      ...
      fromℕ? (𝔽.toℕ a) ≡⟨ fromℕ?Veritas.fromℕ?∘toℕ _ ▹ sym ⟩
      just a ≡⟨ {!!} ⟩
      rms a ∎
      where
  ...
  uip unwords∘w x s c n uj = sym $ begin
    ...
    w∘unwords (x ∷ []) (a 𝕃.All.∷ _) = sym $ begin
      ...
      c∉x⇒wx≡[x] = {!!}
      unwords[] : (s : String) → (unwords ∘ 𝕃.[_]) s ≡ id s
      unwords[] = {!!}
    w∘unwords (x ∷ xs) = {!!}
...
module KanjyVeritas where
  ...
  nilzilcmiv : (x : Buffer)
             → (a b : Buffer.F x)
             → (d : a 𝔽.≤ b)
             → let lb = length ∘ Buffer.lerpinste in
               (_≡_
                 (lb $ proj₁ $ kanji {x} $ Vimcu a b d)
                 (lb x ℕ.∸_ $ suc $ 𝔽.toℕ b ℕ.∸ 𝔽.toℕ a))
  ...
  dropyduv x a b d = sym $ begin
    ...
    teikteikdrop : ∀ {a} → {A : Set a}
                 → (x z : List A)
                 → (n : Fin $ length x)
                 → z ≡ 𝔽.toℕ n ↓ (𝔽.toℕ n ↑ x ++ z)
    teikteikdrop (_ ∷ _) _ 𝔽.zero = refl
    teikteikdrop (_ ∷ xs) _ (𝔽.suc n) = teikteikdrop xs _ n
    open ≡-Reasoning
  ...
  module Cusku where
    ...
    nilzilcmip x a b d = begin
      ...
      BL = Buffer.lerpinste x
      S = a' ↓_ $ suc b' ↑ BL
      lines∘unlines : (lines ∘ unlines) ≗ id
      lines∘unlines = {!!}
      open ≡-Reasoning
